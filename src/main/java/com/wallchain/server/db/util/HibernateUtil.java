...
public class HibernateUtil {
  ...
  private static void init() {
    if (sessionFactory == null) {
      sessionFactory = buildSessionFactory(
          "com/wallchain/server/scripts/hibernate-mysql.conf.xml");
    }
  }
  ...
  private static void setConnectionUrl(final Configuration configuration) {
    ...
    if (HttpSnoopServer.LIVE) {
      connectionUrlValue = "jdbc:mysql://localhost/wallchaindb";
      configuration.setProperty(connectionUrlKey, connectionUrlValue);
    } else {
      connectionUrlValue = "jdbc:mysql://localhost/wallchaintestdb";
      configuration.setProperty(connectionUrlKey, connectionUrlValue);
    }
